...
class HighScores
{
    ...
    {
        return match ($name) {
            'latest'        => end($this->scores),
            'personalBest'  => max($this->scores),
            'topThree'      => $this->getPersonalTopThree(),
            default         => null,
        };
    }


    private function getPersonalTopThree(): array
    {
        $sortedScores = $this->scores;
        rsort($sortedScores);
        return array_slice($sortedScores, 0, 3);
    }
